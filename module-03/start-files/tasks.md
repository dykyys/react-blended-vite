...
домашню сторінку, використайте компонент `Navigate`
...
Для створення сітки використовуй компоненти `<Grid>` та `<GridItem>` всередені
компонента `CountryList`\
...
Додати функціонал для кнопки `Повернутись назад`
